...
if(isset($_POST["status"]) && !empty($_POST["status"])){ 
    ...
    $cond .= " c.status='".$_SESSION['application']['post_status']."'";
}
...
if(isset($_POST["customerID"]) && !empty($_POST["customerID"])){
  // echo $_POST["customerID"]; die('-test');
...
if( (isset($_POST["dateFrom"]) && !empty($_POST["dateFrom"])) || (isset($_POST["dateTo"]) && !empty($_POST["dateTo"])) ) {
     ...
     $cond .= " disbursal_date BETWEEN '".date("Y-m-d",strtotime($_POST["dateFrom"]))."' AND  '".date("Y-m-d",strtotime($_POST["dateTo"]))."'";
  }
  else if(!isset($p_cond)|| $p_cond==''){
...
if(isset($dateFrom) && isset($dateTo)){ 
    ...
    if(!empty($dateFrom)){
      $_SESSION['application']['dateFrom']=$dateFrom;
    }
    if(!empty($dateTo)){
      $_SESSION['application']['dateTo']=$dateTo;
    }
    if(!empty($dateFrom)){
    $cond .= " disbursal_date BETWEEN '".date("Y-m-d",strtotime($dateFrom))."' AND  '".date("Y-m-d",strtotime($dateTo))."'";
  }
}
else if(!isset($p_cond)|| $p_cond==''){
    unset($_SESSION['application']['dateFrom']);
    unset($_SESSION['application']['dateTo']); 
  }
...
/*if(isset($_POST["customerID"]) && !empty($_POST["customerID"])){
  ...
  //for testing...
  echo  $query  = "select  c.name, c.mobile, c.customer_id, c.change_date, c.applied_date, c.pancard, cl.loan_amount, cl.emi, cl.disbursal_date, cl.lender_name, cl.customer_id as l_customer_id, c.status from customer as c left join customer_loan as cl on (c.customer_id=cl.customer_id) ".$cond." LIMIT ".$offset.", ".$page; die('-test');
...
// $query  = "select * from customer ".$cond." LIMIT ".$offset.", ".$page;
 // $query  = "select * from customer as c left join customer_loan as cl on c.customer_id=cl.customer_id ".$cond." LIMIT ".$offset.", ".$page;
 $query = "select  c.name, c.mobile, c.customer_id, c.pancard, cl.loan_amount, cl.emi, cl.disbursal_date, cl.lender_name, cl.customer_id as l_customer_id from customer_loan as cl left join customer as c on (cl.customer_id=c.customer_id) ".$cond." LIMIT ".$offset.", ".$page;
...
$query  = "select count(*) as size from customer_loan as cl left join customer as c on cl.customer_id=c.customer_id ".$cond;
$result = $con->query($query); 
$employee_size = array();
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="filter-box">
            <form id="filter-form-application" class="filter-form" action="employee-loans.php" method="post">
              ...
              <div class="form-group">
                  <input type="tel" name="customerID" class="form-control" placeholder="Customer ID" 
                  <?php if(isset($_SESSION['application']['post_customerID'])){?> value="<?php echo $_SESSION['application']['post_customerID']; ?>" <?php } ?>/>
              </div>
              <div class="form-group">
            ...
            <div class="table-responsive">
                <table class="table table-borderless">
                  ...
                  <tbody>                    
                    ...
                    <?php foreach($employee_loan as $employee_detail) { ?>
                    <tr>
                      <td><a href="loan-profile.php?id=<?php echo $employee_detail['customer_id']; ?>"><?php echo $employee_detail['name']; ?></a></td>
                      <td><?php echo $employee_detail['pancard']; ?></td>
                      <td><?php echo $employee_detail['mobile']; ?></td>
                      <td><?php echo $employee_detail['loan_amount']; ?></td>
                      
                      
                      <td><?php echo $employee_detail['emi']; ?></td>
                      <td><?php echo $employee_detail['disbursal_date']; ?></td>
                      <!-- <td><?php //echo $status[$employee_detail['status']]; ?></td> -->
                      <td><img src="images/bank_images/bank_logo.png" alt=""/></td>
                    </tr>
              ...
              <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-center">
                      <li class="page-item <?php if($p<=1){ echo 'disabled'; } ?> ">
                        <a class="page-link" href="?p=<?php echo $p-1;?>&from=<?php echo $dateFrom ?>&to=<?php echo $dateTo; ?>"  tabindex="-1">Previous</a>
                      </li>
                      <?php
                        ...
                        {
                      ?>
                      <li class="page-item <?php if(isset($p) && $i==$p){echo "active"; } ?> "><a class="page-link" href="?p=<?php echo $i; ?>&from=<?php echo $dateFrom; ?>&to=<?php echo $dateTo; ?>"><?php echo $i; ?></a></li>
                       <?php
                         }
                       ?>
                      <li class="page-item <?php if($p>=$perpage_record-1){ echo 'disabled'; } ?>" >
                        <a class="page-link" href="?p=<?php echo $p+1;?>&from=<?php echo $dateFrom ?>&dateTo=<?php echo $to; ?>">Next</a>
                      </li>
                    </ul>
              </nav>
              <?php } ?>
